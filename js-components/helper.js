...
function hasClass(el, className) {
    ...
    return _.includes(el.className.split(/\s+/), className)
}
...
function getParentNode(node, className) {
    var res = node;
    while (res && !hasClass(res, className)) {
        res = res.parentNode
    }
    return res
}
...
function setStyle(elem, type, value) {
    if (_props[type]) {
        //兼容ie9
        // console.log(value)
        elem.style.setProperty(_props[type], (getIEVersion() != 9 ? value : value.toString()), "important");
    } else {
...
function transformCoordinateBySender(value, coordinate, dimension) {
    dimension = dimension || 'x'
    value = value || 0
    if (dimension == 'x') {
        return STANDARD_COORDINATE_SIZE[0] * value / coordinate[0]
    } else {
        return STANDARD_COORDINATE_SIZE[1] * value / coordinate[1]
    }
}
...
const layoutSize = {
    ...
    chartRoomMargin: 20,
    mainPadding: 5,
    standardScreenSize: [1920, 1080],
    standardModuleWidth: 260
}

function calculateViewport() {
    var viewport;
    //
    document.getElementById('state-modules').style.setProperty('width', `${Math.max(225, window.innerWidth * layoutSize.standardModuleWidth / layoutSize.standardScreenSize[0])}px`)
...
function getPointsOfPath(d) {
    if (!_.isString(d)) {
        return []
    }
    var segStr = _.compact(d.split(/[ML]/))
    return _.map(segStr, seg => seg.split(/\s+/))
}

function getStyle(_obj, attr) {　　
    _obj = _.isElement(_obj) ? _obj : document.querySelector(_obj);　　
...
function isWebRTCSupported() {
    return window.WebAssembly && window.RTCPeerConnection && navigator.mediaDevices && navigator.mediaDevices.getUserMedia && myBrowser() == 'Chrome'
}
...
function resolveStaticPath(path) {
    ...
    return `${PRODUCTION ? GLOB.STATIC_PATH : '/'}${path.replace(/^\//, '')}`
}
...
function blink({
    ...
    return function innner() {
        ...
        setTimeout(() => {
            times += 1
            if (times <= maxTime) {
                // console.log(getStyle(elHandUp, 'visibility'))
                onBlink.call({
                    el,
                    frequency,
                    maxTime,
...
function smoothUICons(data, process, {
    context,
    onComplete,
    segSize = 1600
}) {
    var segCount = Math.ceil(data.length / segSize)
    // for (let i = 0; i < segCount; i++) {
    let i = 0
    var f = () => {
        for (let j = 0; j < segSize; j++) {
            if ((i * segSize + j) < data.length) {
                // console.log(i * segSize + j)
                process.call(context, data[i * segSize + j])
            } else {
...
//拖拽
function dragFunc(el, parent) {
    var Drag = el;
    Drag.onmousedown = function(event) {
        var ev = event || window.event;
        event.stopPropagation();
        var disX = ev.clientX - Drag.offsetLeft + (parent ? parent.offsetLeft : 0);
        var disY = ev.clientY - Drag.offsetTop + (parent ? parent.offsetTop : 0);
...
function rankList(list, field) {
    var g = _.map(_.groupBy(list, field), (gv, gk) => ({
        gv,
        gk
    }))
    var sortedG = _.sortBy(g, d => parseInt(d.gk) * -1)
    return _.reduce(sortedG, (res, sg, i) => {
        return res.concat(_.map(sg.gv, gvd => {
            gvd.rank = (i + 1)
            return gvd
        }))
    }, [])
...
function setElVisible(elInstance, visible) {
    elInstance.visible = visible
    elInstance.wrapper.style.setProperty('display', elInstance.visible ? 'block' : 'none')
}
...
function setTimeoutEffectively(cb, duration){
  let lastTime = now();
  function loop(){
